...
export class AddReportComponent implements OnInit {
  ...
  get attachments(): FormArray {
    return this.reportForm.get('attachments') as FormArray;
  }

  get files(): FormArray {
    return this.reportForm.get('files') as FormArray;
  }
    ...
    this.reportsService.getReportById(id).subscribe({
      next: (report: Report) => {
        ...
        if (report.attachments?.length) {
          report.attachments.forEach(a => this.addAttachment(a));
        }
  ...
  saveReport(): void {
    ...
    const request = this.editMode
      ? this.reportsService.editReport(this.report!._id, fd)
      : this.reportsService.addReport(values.eventId, fd);

    request.subscribe({
      next: () => {
        Swal.fire({
          title: this.editMode ? 'Reporte actualizado' : 'Reporte creado',
          icon: 'success',
          confirmButtonColor: '#3f51b5'
        }).then(() => this.goBack());
      ...
      error: (err) => {
        ...
        Swal.fire({
          title: 'Error',
          text: err?.error?.message || 'No se pudo procesar el reporte.',
          icon: 'error',
